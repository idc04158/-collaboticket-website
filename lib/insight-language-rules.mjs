...
const JP_CHAR = /[\u3040-\u30ff\u3400-\u9fff\u30a0-\u30ff]/
const JP_CHAR_GLOBAL = /[\u3040-\u30ff\u3400-\u9fff\u30a0-\u30ff]/g
const HANGUL = /[\uac00-\ud7a3]/
...
export function scanInsightLanguageMix(text, options = {}) {
  ...
  const isGlossary = (options.slug ?? "") === "japan-ecommerce-marketing-glossary"
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isKeywordMap && (line.includes("|") || line.includes("成分") || line.includes("「"))) continue

    const remainder = stripAllowedJapanese(line)
    if (!JP_CHAR.test(remainder)) continue

    const jpMatches = remainder.match(JP_CHAR_GLOBAL)
    issues.push({
      line: i + 1,
